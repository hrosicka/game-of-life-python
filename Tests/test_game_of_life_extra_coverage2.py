...
# Helper: dynamically loads a module and class from a given file path
...
def test_glider_set_initial_patterns_and_text():
    module, GliderSimulation = load_module_and_class(
        "game-of-life-glider.py", "GliderSimulation"
    )
...
def test_lwss_run_handles_keyboard_interrupt_and_no_main_side_effects(monkeypatch):
    ...
    game = LWSS({"width": 10, "height": 6, "delay_seconds": 0.0})
    # run() should handle KeyboardInterrupt gracefully (not raise)
    game.run()

    assert isinstance(game.console, DummyConsole)
    # After run, at least one message should exist (start/stop)
    assert len(game.console.messages) >= 0


def test_imports_do_not_execute_main_blocks():
    # Ensure importing modules does not create module-level runtime objects
    # (main logic should be protected by if __name__ == "__main__")
    file_list = [
        "game-of-life-gun.py",
        "game-of-life-lwss.py",
        "game-of-life-glider.py",
        "game-of-life-pulsar.py",
    ]
    ...
    for fname in file_list:
        ...
        # Common script-level names used under __main__: 'game', 'sim' etc.
        # They should not exist after import.
